...
# Joining data together (they work only with Normal tissue data and the treatment data)

```{r}
normal_dataset_long <- large_dataset %>% 
...
all_data <- treatment_dataset %>% 
  select(-treatment, -sex, -age, -disease_duration, -acc_num) %>% 
  pivot_longer(cols = -c(id),
               names_to = "Genes" , 
               values_to = "log2fc") %>% 
  bind_rows(normal_dataset_long) 
```
...
need to find a way to sort them as I want
```{r}
...
## filtering significantly up/down expressed genes (log2fc > 1.5) 
...
selected_genes_treatment <- transpose_logfold_treatment %>%  
  ...
  filter(mean_log2fc >= 1.5 | mean_log2fc <= -1.5)
